...
void draw() {
  background(0);
  camera(0, 0, 0, width/2, height/2.0, 0, 0, 1, 0);
   //camera(width/2+map(mouseX, 0, width, -2*width, 2*width), 
   //      height/2+map(mouseY, 0, height, -height, height),
   //      height/2/tan(PI*30.0 / 180.0), 
   //      width, height/2.0, 0, 0, 1, 0);   
  pushMatrix();
  rotateY(radians(map(mouseX+(width/2), 0, width, 0, 360)));
  rotateX(radians(map(mouseY+(height/2), 0, height, 0, 360)));
  translate(width/2, height/2, 0);  
  textureSphere(410 , 400, 400, img);
  popMatrix();
}
